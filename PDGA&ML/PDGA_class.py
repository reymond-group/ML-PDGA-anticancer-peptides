...
class PDGA:
    ...
    def connect_mol(self, mol1, mol2, metbond):
        ...
        for i in range(count):
            ...
            if metbond == True and self._methyl == True:
                Nterm = []
                Met = []
        ...
        for gen in gs:
            ...
            for aa in gen:
                ...
                if molecule == '':
                    ...
                    molecule = self.connect_mol(molecule, rdmolfiles.MolFromSmiles(self.AA_SMILES[aa]), metbond)
            ...
            if bs:
                ...
                if molecule == '':
                    ...
                    molecule = self.connect_mol(molecule, rdmolfiles.MolFromSmiles(self.B_SMILES[bs[0]]), metbond)
                bs.pop(0)
    ...
    def make_new_gen_cyclic(self, n):
        ...
        while len(new_gen) < n / 2:
            ...
            child = child.replace("-Z", "Z").replace("-z", "z").replace("-P", "P").replace("-p", "p")
    ...
    def mutate(self, gen):
        ...
        if self._methyl and mutation == 'M10':
        ...
        if self._methyl and mutation == 'M11':
    ...
    def mutate_cyclic(self, gen):
        ...
        if self._methyl and mutation == 'M12':
            seqs_demethylate = np.random.choice(gen, int(round(len(gen) * self.mut_rate, 0)), replace=False)
            for seq in gen:
                if seq in seqs_demethylate:
                    gen_tmp.append(self.demethylate(seq))
