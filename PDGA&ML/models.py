...
class Classifier:
    def __init__(self, embedding_size, hidden_size, num_layers, vocabulary):
        ...
        self.model = ClassifierRNN(len(self.vocabulary), embedding_size, hidden_size, num_layers, 2)
    ...
    def load_from_file(cls, file_path: str):
        ...
        model = ClassifierRNN(len(vocabulary), embedding_size, hidden_size, num_layers, 2)
        model.load_state_dict(save_dict["model"])
        #model.to(device)
        
        loaded_model = Classifier(embedding_size, hidden_size, num_layers, vocabulary)
